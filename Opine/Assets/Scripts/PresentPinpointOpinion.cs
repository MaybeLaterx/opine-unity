...
public class PresentPinpointOpinion : MonoBehaviour {
    ...
    public float enterTime = 2f, myAnswerTime = 2f, opponentAnswerTime = 2f, realAnswerTime = 2f, starTime = 2f, moveTime = 2f;
    ...
    void Start () {
        ...
        for (int question = 0; question < questionsPerRound; question++)
        {
            ...
            float timeToWait = wholeQuestionTime * question;
            ...
            // CREATIONS + EASING
            // my graph 
            StartCoroutine(CreateGraph(new Vector3(-4f, -6.5f, 0f), "MyAnswer", myColour, myAnswer, timeToWait)); 
            // opponent graph (if versus)
            if (versus)
            {
                StartCoroutine(CreateGraph(new Vector3(4f, -6.5f, 0f), "OpponentAnswer", opponentColour, opponentAnswer, timeToWait));
            }
            // real graph 
            StartCoroutine(CreateGraph(new Vector3 (0f, -6.5f, 0f), "RealAnswer", realColour, realAnswer, timeToWait));
            // topic card + text
            ...
            Sprite myStar = DetermineStar(myErrorMargin); 
            ...
            Sprite opponentStar = DetermineStar(opponentErrorMargin);
        ...
        StartCoroutine(LoadNextLevel(wholeQuestionTime * questionsPerRound));
    ...
    Sprite DetermineStar(float errorMargin)
    {

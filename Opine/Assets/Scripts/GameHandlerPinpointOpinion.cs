...
public class GameHandlerPinpointOpinion : MonoBehaviour {
	...
	void Start () {
        ...
        graph = UtilitiesScript.CreatePinpointOpinionGraph(graphPrefab, json["data"][round][question][0]["id"], "0.5");

        // Create indicators
        indicators = UtilitiesScript.CreateIndicators(round, indicatorPrefab);
        string[] defaultAnswer = { "0.5" };
        foreach (Transform indicator in indicators)
        {
            indicator.GetComponent<IndicatorColour>().myAnswers = defaultAnswer;
        }
    ...
    IEnumerator LoadLevelDelay(string level, float delay)
    {
        yield return new WaitForSeconds(delay);
        Application.LoadLevel(level); 
    }

    public void InputAnswer(bool isSkip)
    {
        // Move graph right
        graph.GetComponent<Ease>().alignmentX = 22f;//12f;
        Destroy(graph.gameObject, 0.5f);
        ...
        // Update lights 
        bool isGameOver = UpdateIndicators(isSkip);

        // Get next, if required
        if (!isGameOver)
        {
            string prevPercentage = indicators[question].GetComponent<IndicatorColour>().myAnswers[0];
            print("PrevPercentage: " + prevPercentage);
            StartCoroutine(CreateGraph(graphPrefab, json["data"][round][question][0]["id"], prevPercentage, 0.6f));
        }
    }
    ...
    bool UpdateIndicators(bool isSkip)
    {
        ...
        float percFloat = graph.GetComponent<GraphSelect>().ratio;
            //string percString = UtilitiesScript.DecimalToPercentage(percFloat);

        // Skipping or answering?
        if (isSkip) print("Skipping!");
        else
        {
        ...
        thisLight.GetComponent<IndicatorColour>().myAnswers = new string[] { percFloat.ToString() }; // "0.34", "0.67" etc
        ...
        do
        {
            ...
            if (i > totalQuestions) // every question is answered
            {
                ...
                FetchFullGameTopics.round4 = compiledAnswers; // 1 indexed
